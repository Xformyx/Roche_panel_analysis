...
_find_file() {
    local pat="$1"
    find "$PATCH_DIR" -maxdepth 1 -name "$pat" 2>/dev/null | sort -V | tail -1
}
...
if [[ "$MODE" = "patch" ]]; then
    ...
    echo "  (복원 방법: cp $BACKUP_DIR/<파일명> $INSTALL_DIR/해당경로/)"
else
    step "디렉터리 준비"
fi
...
if [[ "$SKIP_DATA" = true ]]; then
    ...
    for cand in \
        ...
        "$(dirname $PATCH_DIR)/roche_data_hg38.tar"  \
        "$(dirname $PATCH_DIR)/roche_data_hg19.tar"  \
    ...
    if [[ ${#DATA_TARS[@]} -eq 0 ]]; then
        if [[ -d "$DATA_DIR/refs" ]] && [[ -n "$(ls -A $DATA_DIR/refs 2>/dev/null)" ]]; then
            ...
            warn "  roche_data_hg38.tar  roche_data_hg19.tar  rna_refs.tar.gz"
        fi
    else
        _ensure_dir "$DATA_DIR"
        for tar_file in "${DATA_TARS[@]}"; do
            info "압축 해제: $(basename $tar_file) → $DATA_DIR"
            case "$tar_file" in
                *.tar.gz|*.tgz) tar -xzf "$tar_file" -C "$DATA_DIR" --strip-components=1 ;;
                *.tar)          tar -xf  "$tar_file" -C "$DATA_DIR" --strip-components=1 ;;
            esac
...
[[ -f "$COMPOSE_FILE" ]] || die "docker-compose.prod.yml 을 찾을 수 없습니다: $COMPOSE_FILE"
